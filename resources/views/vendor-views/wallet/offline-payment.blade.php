...
{{-- {{ dd($data->method_informations) }} --}}
        ...
        <div class="card">
            <div class="card-header py-2 border-0">
                ...
                <h5 class="card-title">{{translate('messages.offline_payment_information')}}<span class="badge badge-soft-dark ml-2" id="itemCount">{{ \App\CentralLogics\Helpers::format_currency(abs($wallet->collected_cash)) }}</span></h5>
                </div>
            </div>
            
            <div class="card mb-3">
                <div class="card-body">
                    <form action="{{route('vendor.wallet.offline-payment')}}" method="post">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input id="{{ $method['customer_input'] }}" type="text" name="{{ $method['customer_input'] }}" class="form-control"
                        ...
                        <div class="btn--container justify-content-end">
                            <button type="submit" class="btn btn--primary">{{translate('messages.pay_offline')}}</button>
                        </div>
                    </form>
                </div>
            </div>
